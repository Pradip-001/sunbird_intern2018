...
telemetrySyncManager.prototype.sync = function (callback) {
  if (this.teleData.length > 0) {
    ...
    fs.writeFile('./telemetry_log.txt', 'Telemetry Sync Starting...\n', function (err) {
  if (err) return console.log(err);
  console.log('Telemetry Sync Starting... > ./telemetry_log.txt');
});
    console.log('Sync Default')

    request(options, function (err, res, body) {
      if (body && body.params && body.params.status === 'successful') {
        self.teleData.splice(0, self.config.batchsize)
       
fs.writeFile('./0telemetry_log.txt', 'Telemetry Submitted Successfully...'+"\n", function (err) {
  if (err) return console.log(err);
  console.log('Telemetry Submitted successfully... > ./telemetry_log.txt');
});
...
fs.writeFile('./telemetry_log.txt', 'TELEMETRY SYNC FAILED, DUE TO:', function (err) {
  if (err) return console.log(err);
  console.log('Telemetry Sync Failed... > ./telemetry_log.txt');
});

        fs.writeFile('./telemetry_log.txt', JSON.stringify(body)+"\n", function (err) {
  if (err) return console.log(err);
  console.log('Telemetry failed data.... > ./telemetry_log.txt');
});

        console.log('Node_modules sync manager')
